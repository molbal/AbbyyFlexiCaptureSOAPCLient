...
/**
 * This class stands for roleIds StructType
 * @package Abbyy
 * @subpackage Structs
 */
class AbbyyRoleIds extends AbstractStructBase
{
    /**
     * The int
    ...
    public $int;
    /**
     * Constructor method for roleIds
     * @uses AbbyyRoleIds::setInt()
     * @param int[] $int
     */
    ...
    public static function validateIntForArrayConstraintsFromSetInt(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $roleIdsIntItem) {
            // validation for constraint: itemType
            if (!(is_int($roleIdsIntItem) || ctype_digit($roleIdsIntItem))) {
                $invalidValues[] = is_object($roleIdsIntItem) ? get_class($roleIdsIntItem) : sprintf('%s(%s)', gettype($roleIdsIntItem), var_export($roleIdsIntItem, true));
            }
        }
     ...
     * @param int[] $int
     * @return \AbbyyBridge\StructType\AbbyyRoleIds
     */

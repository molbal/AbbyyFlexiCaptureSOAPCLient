...
/**
 * This class stands for stageExtIds StructType
 * @package Abbyy
 * @subpackage Structs
 */
class AbbyyStageExtIds extends AbstractStructBase
{
    /**
     * The int
    ...
    public $int;
    /**
     * Constructor method for stageExtIds
     * @uses AbbyyStageExtIds::setInt()
     * @param int[] $int
     */
    ...
    public static function validateIntForArrayConstraintsFromSetInt(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $stageExtIdsIntItem) {
            // validation for constraint: itemType
            if (!(is_int($stageExtIdsIntItem) || ctype_digit($stageExtIdsIntItem))) {
                $invalidValues[] = is_object($stageExtIdsIntItem) ? get_class($stageExtIdsIntItem) : sprintf('%s(%s)', gettype($stageExtIdsIntItem), var_export($stageExtIdsIntItem, true));
            }
        }
     ...
     * @param int[] $int
     * @return \AbbyyBridge\StructType\AbbyyStageExtIds
     */

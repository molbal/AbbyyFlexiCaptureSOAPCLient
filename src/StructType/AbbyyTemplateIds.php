...
/**
 * This class stands for templateIds StructType
 * @package Abbyy
 * @subpackage Structs
 */
class AbbyyTemplateIds extends AbstractStructBase
{
    /**
     * The int
    ...
    public $int;
    /**
     * Constructor method for templateIds
     * @uses AbbyyTemplateIds::setInt()
     * @param int[] $int
     */
    ...
    public static function validateIntForArrayConstraintsFromSetInt(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $templateIdsIntItem) {
            // validation for constraint: itemType
            if (!(is_int($templateIdsIntItem) || ctype_digit($templateIdsIntItem))) {
                $invalidValues[] = is_object($templateIdsIntItem) ? get_class($templateIdsIntItem) : sprintf('%s(%s)', gettype($templateIdsIntItem), var_export($templateIdsIntItem, true));
            }
        }
     ...
     * @param int[] $int
     * @return \AbbyyBridge\StructType\AbbyyTemplateIds
     */
